...
type Stream = { url: string; kind: string; server: string };
type Episode = { id: string; name: string; number: number; pageUrl: string; streamUrl?: string; current: boolean };
type Analysis = { title: string; pageUrl: string; streams?: Stream[] | null; episodes: Episode[]; htmlBytes: number; sourceLabel: string };
type MovieEntry = { key: string; source: string; analysis: Analysis; outputDir: string };
type QueueEvent = {
    id: string; movie: string; index: number; total: number; name: string;
    status: 'resolving' | 'downloading' | 'completed' | 'failed' | 'skipped';
    output?: string; message?: string; completed: number; failed: number; skipped: number;
};
type ProgressEvent = { index: number; total: number; name: string; time: string; speed: string };
type DoneEvent = { total: number; completed: number; failed: number; skipped: number; cancelled: boolean };
...
function episodeKey(movieKey: string, episodeID: string) {
...
function App() {
    ...
    const availableServers = useMemo(() => {
        ...
        movies.forEach(movie => (movie.analysis.streams ?? []).forEach(stream => {
        ...
        try {
            await StartDownload({
                ...
                items: selectedEntries.map(({movie, episode, key}) => ({
                    ...
                    streamUrl: episode.streamUrl || '', title: movie.analysis.title, outputDir: movie.outputDir,
                })),
            } as any);
            setRunning(true);
            setPaused(false);
                                ...
                                {!isCollapsed && <div className="movie-episodes">
                                    {movie.analysis.episodes.map(episode => {
                                        ...
                                        return <label className={`episode-row ${selected.has(key) ? 'selected' : ''} ${state ? `status-${state.status}` : ''}`} key={key}>
                                            ...
                                            <span className="episode-name">{episode.name}{episode.current && <em>đang xem</em>}</span>
